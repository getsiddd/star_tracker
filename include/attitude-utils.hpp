...
class Mat4; // define above so we can use in Vec3 class

/// Four dimensional vector with decimaling point components
class Vec4 {
    public:
        float a;
        float b;
...
void SerializeVec2(SerializeContext *, const Vec2 &);
...
void SerializeVec4(SerializeContext *, const Vec4 &);
...
public:
    ...
    Vec3 Rotate(const Vec3 &) const;
    bool IsKnown() const;
    std::string name = "attitude131221";
