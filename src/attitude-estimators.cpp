 ...
 */
float QuestCharPoly(float x, float a, float b, float c, float d, float s) {return (std::pow(x,2)-a) * (std::pow(x,2)-b) - (c*x) + (c*s) - d;}

/**
 * Derivitive of the characteristic polynomial of the quest K-matrix
 */
float QuestCharPolyPrime(float x, float a, float b, float c) {return 4*std::pow(x,3) - 2*(a+b)*x - c;}
...
Attitude QuestAlgorithm::Go(const Camera &camera,
    ...
    float a = std::pow(sigma,2) - kappa;
    float b = std::pow(sigma,2) + (Z * Z);
    float c = delta + (Z * S * Z);
    float d = Z * (S * S) * Z;
    ...
    float alpha = std::pow(eig,2) - std::pow(sigma, 2) + kappa;
    float beta = eig - sigma;
    float gamma = (eig + sigma) * alpha - delta;

    Vec3 X = ((kIdentityMat3 * alpha) + (S * beta) + (S * S)) * Z;
    float scalar = 1 / std::sqrt(std::pow(gamma,2) + X.MagnitudeSq());
    X = X * scalar;
    gamma *= scalar;
