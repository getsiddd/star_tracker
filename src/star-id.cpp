...
StarIdentifiers GeometricVotingStarIdAlgorithm::Go(
    ...
    for (int i = 0; i < (int)identified.size(); i++) {
        ...
        for (int j = i + 1; j < (int)identified.size(); j++) {
            ...
            if (std::abs(sDist - cDist) < tolerance) {
                verificationVotes[i]++;
                verificationVotes[j]++;
            }
        }
...
float IRUnidentifiedCentroid::VerticalAnglesToAngleFrom90(float v1, float v2) {
    return std::abs(DecimalModulo(v1-v2, M_PI) - M_PI_2);
}
...
void IRUnidentifiedCentroid::AddIdentifiedStar(const StarIdentifier &starId, const Stars &stars) {
    ...
    float angleFromVertical = std::atan2(positionDifference.y, positionDifference.x);
...
std::vector<std::vector<IRUnidentifiedCentroid *>::iterator> FindUnidentifiedCentroidsInRange(
    ...
    float minCos = std::cos(maxDistance);
    float maxCos = std::cos(minDistance);
...
StarIdentifiers PyramidStarIdAlgorithm::Go(
    ...
    if (databaseBuffer == NULL || stars.size() < 4) {
        BOOST_LOG_TRIVIAL(error) << "Not enough stars, or database missing.";
        return identified;
    }
    DeserializeContext des(databaseBuffer);
    PairDistanceKVectorDatabase vectorDatabase(&des);
    ...
    float expectedMismatchesConstant = std::pow(numFalseStars, 4) * std::pow(tolerance, 5) / 2 / std::pow(M_PI, 2);
        ...
        for (int kIter = 0; kIter < kMax; kIter++) {
            ...
            for (int rIter = 0; rIter < rMax; rIter++) {
                ...
                for (int iIter = 0; iIter <= iMax; iIter++) {
                    ...
                    float iSinInner = std::sin(Angle(jSpatial - iSpatial, kSpatial - iSpatial));
                    float jSinInner = std::sin(Angle(iSpatial - jSpatial, kSpatial - jSpatial));
                    float kSinInner = std::sin(Angle(iSpatial - kSpatial, jSpatial - kSpatial));
                    ...
                    if (expectedMismatches > maxMismatchProbability) {
                        BOOST_LOG_TRIVIAL(info) << "skip: mismatch prob.";
                        continue;
                    }
...
#undef _CHECK_DISTANCE
                    ...
                    if (iMatch != -1) {
                        ...
                        BOOST_LOG_TRIVIAL(info) << "Matched unique pyramid!";
                        BOOST_LOG_TRIVIAL(info) << "Expected mismatches: " << std::scientific << expectedMismatches << std::fixed;
                        ...
                        BOOST_LOG_TRIVIAL(info) << "Identified an additional " << numAdditionallyIdentified << " stars.";
