...
Quaternion::Quaternion(const Vec3 &input, float theta) {
    real = std::cos(theta/2);
    // the compiler will optimize it. Right?
    i = input.x * std::sin(theta/2);
    j = input.y * std::sin(theta/2);
    k = input.z * std::sin(theta/2);
}
...
float Quaternion::Angle() const {
    ...
    return (real >= 1 ? 0 : std::acos(real))*2;
}
...
void Quaternion::SetAngle(float newAngle) {
    real = std::cos(newAngle/2);
    SetVector(Vector().Normalize() * std::sin(newAngle/2));
}
...
EulerAngles Quaternion::ToSpherical() const {
    ...
    float ra = std::atan2(2*(-real*k+i*j), 1-2*(j*j+k*k));
    if (ra < 0)
        ra += 2*M_PI;
    float de = -std::asin(2*(-real*j-i*k)); // allow de to be positive or negaive, as is convention
    float roll = -std::atan2(2*(-real*i+j*k), 1-2*(i*i+j*j));
    if (roll < 0)
        roll += 2*M_PI;
...
Vec3 SphericalToSpatial(float ra, float de) {
    return {
        std::cos(ra)*std::cos(de),
        std::sin(ra)*std::cos(de),
        std::sin(de),
    };
}
...
float DecimalModulo(float x, float mod) {
    // first but not last chatgpt generated code in lost:
    float result = x - mod * std::floor(x / mod);
...
float Vec2::MagnitudeSq() const {
    return std::fma(x,x, y*y);
}

float Vec3::Magnitude() const {
    return std::hypot(std::hypot(x, y), z); // not sure if this is faster than a simple sqrt, but it does have less error?
}
...
Quaternion DCMToQuaternion(const Mat3 &dcm) {
    ...
    assert(std::abs(newXAxis.Magnitude()-1) < 0.001);
...
void SerializeVec2(SerializeContext *ser, const Vec2 &vec) {
    SerializePrimitive<float>(ser, vec.x);
    SerializePrimitive<float>(ser, vec.y);
}

Vec2 DeserializeVec2(DeserializeContext *des) {
    Vec2 result = {
...
/// Serialize a Vec3 to buffer. Takes up space according to SerializeLengthVec3
void SerializeVec4(SerializeContext *ser, const Vec4 &vec) {
    SerializePrimitive<float>(ser, vec.a);
    SerializePrimitive<float>(ser, vec.b);
    SerializePrimitive<float>(ser, vec.c);
    SerializePrimitive<float>(ser, vec.d);
}

Vec4 DeserializeVec4(DeserializeContext *des) {
    Vec4 result = {
        DeserializePrimitive<float>(des),
        DeserializePrimitive<float>(des),
        DeserializePrimitive<float>(des),
...
float AngleUnit(const Vec3 &vec1, const Vec3 &vec2) {
    ...
    return dot >= 1 ? 0 : dot <= -1 ? M_PI-0.0000001 : std::acos(dot);
}
