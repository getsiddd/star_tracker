...
int BasicThreshold(unsigned char *image, int imageWidth, int imageHeight) {
    ...
    for (long i = 0; i < totalPixels; i++) {
        totalMag = totalMag + (long)image[i];
    }
...
Stars IterativeWeightedCenterOfGravityAlgorithm::Go(unsigned char *image, int imageWidth, int imageHeight) const {
    ...
    for (long i = 0; i < imageHeight * imageWidth; i++) {
        ...
        if (image[i] >= p.cutoff && p.checkedIndices.count(i) == 0) {
            ...
            fwhm = std::sqrt(count);
            standardDeviation = fwhm / (2.0 * std::sqrt(2.0 * std::log(2.0)));
            float modifiedStdDev = 2.0 * std::pow(standardDeviation, 2);
            // TODO: Why are these decimals? --Mark
            float guessXCoord = (p.guess % imageWidth);
            float guessYCoord = (p.guess / imageWidth);
            ...
            //traverse through star indices, calculate W at each coordinate, add to final coordinate sums
                ...
                for (long j = 0; j < (long)starIndices.size(); j++) {
                    //calculate w
                    float currXCoord = starIndices.at(j) % imageWidth;
                    float currYCoord = starIndices.at(j) / imageWidth;
                    w = p.maxIntensity * std::exp((-1.0) * ((std::pow(currXCoord - guessXCoord, 2) / modifiedStdDev) + (std::pow(currYCoord - guessYCoord, 2) / modifiedStdDev)));

                    xWeightedCoordMagSum += w * currXCoord * (image[starIndices.at(j)]);
                    yWeightedCoordMagSum += w * currYCoord * (image[starIndices.at(j)]);
                    weightedMagSum += w * (image[starIndices.at(j)]);
                }
            ...
            if (p.isValid) {
                result.push_back(Star(guessXCoord + (0.5), guessYCoord + (0.5), xDiameter/2.0, yDiameter/2.0, starIndices.size()));
            }
        }
    }
    return result;
