...
UserSpecifiedOutputStream::UserSpecifiedOutputStream(std::string filePath, bool isBinary, bool isContinousWrite) {
    if (isBinary && isatty(fileno(stdout)) && (filePath == "stdout" || filePath == "-")) {
        BOOST_LOG_TRIVIAL(error) << "WARNING: output contains binary contents. Not printed to terminal.";
...
MultiDatabaseDescriptor GenerateDatabases(const Catalog &catalog, const DatabaseOptions &values) {
    ...
    dbEntries.emplace_back(kStarCatalogMagicValue, catalogSer.buffer);
    ...
    if (values.kvector) {
        ...
        BOOST_LOG_TRIVIAL(error) << "No database builder selected -- no database generated.";
        exit(1);
    }
...
MultiDatabaseDescriptor GenerateQuadsDatabases(const CatalogQuads &catalogQuads, const DatabaseOptions &values) {
    MultiDatabaseDescriptor dbEntries;

    SerializeContext catalogSer = serFromDbValues(values);
    
    // TODO decide why we have this inclMagnitude and inclName and if we should change that
    SerializeCatalogQuads(&catalogSer, catalogQuads, false, true);
    BOOST_LOG_TRIVIAL(info) << "Total Quads generated : " << catalogQuads.size();  
    dbEntries.emplace_back(kQuadCatalogMagicValue, catalogSer.buffer);
    BOOST_LOG_TRIVIAL(info) << "Total Quads generated : " << catalogQuads.size();  
    return dbEntries;
}

void GenerateQuadsDatabaseToFile(const CatalogQuads &quads, const DatabaseOptions &values, uint32_t dbFlags, UserSpecifiedOutputStream pos) {
    MultiDatabaseDescriptor dbEntries;
    SerializeContext serQuads = serFromDbValues(values);

    SerializePrimitive<int16_t>(&serQuads, quads.size());

    // flags 
    bool inclMagnitude = false;
    bool inclName = true;
    int8_t flags = (inclMagnitude) | (inclName << 1);
    SerializePrimitive<int8_t>(&serQuads, flags);
}
...
float FocalLengthFromOptions(const PipelineOptions &values, int xResolution) {
    if ((values.pixelSize != -1) ^ (values.focalLength != 0)) {
        BOOST_LOG_TRIVIAL(error) << "ERROR: Exactly one of --pixel-size or --focal-length were set.";
        exit(1);
    }
    ...
    if (values.pixelSize != -1 && values.fov != 20) {
        BOOST_LOG_TRIVIAL(error) << "ERROR: Both focal length and FOV were provided. We only need one of the two methods (pixel size + focal length, or fov) to determine fov, please only provide one.";
        exit(1);
    }
...
cairo_surface_t *PipelineInput::InputImageSurface() {
    Image *inputImage = InputImage();
...
PipelineInputList GetPngPipelineInput(const PipelineOptions &values) {
    ...
    std::string pngPath = values.file;
    //delete cairoSurface;
    return result;
}
...
GeneratedPipelineInput::GeneratedPipelineInput(const Catalog &catalog,
    ...
    int oversamplingPerAxis = std::ceil(std::sqrt(oversampling));
    if (oversamplingPerAxis*oversamplingPerAxis != oversampling) {
        BOOST_LOG_TRIVIAL(error) << "WARNING: oversampling was not a perfect square. Rounding up to " << oversamplingPerAxis*oversamplingPerAxis << ".";
    }
    ...
    for (int i = 0; i < numFalseStars; i++) {
        ...
        float de = std::asin(uniformDistribution(*rng)*2 - 1);
    ...
    for (int i = 0; i < (int)catalogWithFalse.size(); i++) {
        ...
        if (camera.InSensor(camCoords)) {
            ...
            float radius = std::ceil(std::sqrt(-std::log(interestingThreshold/peakBrightnessPerTime/exposureTime)*2*M_PI*starSpreadStdDev*starSpreadStdDev));
    ...
    for (int i = 0; i < image.width * image.height; i++) {
        ...
        if (shotNoise) {
            ...
            if (photons > (LONG_MAX) - (3.0) * std::sqrt(LONG_MAX)) {
                BOOST_LOG_TRIVIAL(info) << "ERROR: One of the pixels had too many photons. Generated image would not be physically accurate, exiting.";
                exit(1);
            }
        ...
        float clampedBrightness = std::max(fmin(curBrightness, 1.0), 0.0);
...
PipelineInputList GetPipelineInput(const PipelineOptions &values) {
    ...
    if (values.mode == "file"){
        if (values.file != "") {
            ...
            BOOST_LOG_TRIVIAL(info) << "No file path provided to read";
            exit(1);
        }
    }
    ...
    if (values.mode == "generated"){
        ...
        BOOST_LOG_TRIVIAL(info) << "No running mode argument has been provided";
        exit(1);
    }
}
...
Pipeline SetPipeline(const PipelineOptions &values) {
    ...
    if (values.databasePath != "") {
        ...
        BOOST_LOG_TRIVIAL(info) << "Reading " << std::to_string(length) << " bytes of database.";
...
std::vector<PipelineOutput> Pipeline::Go(PipelineInputList &inputs) {
            ...
            for (int j = 0;
                ...
                for (const StarIdentifier &starId : *expected[0]->ExpectedStarIds()) {
                    if (starId.starIndex == expectedCentroidIndex) {
                        ...
                        BOOST_LOG_TRIVIAL(info) << "actual_centroid_" << std::to_string(i) << "_expected_id_" << std::to_string(j) << " " << expectedName << std::endl;
                        foundIt = true;
                    }
                }
            }
...
static void PipelineComparatorPlotOutput1(std::ostream &os,
                                         const PipelineInputList &expected,
                                         const std::vector<PipelineOutput> &actual,
                                         const PipelineOptions &) {
...
static void PipelineComparatorPlotOutput(std::ostream &os,
                                         ...
                                         const PipelineOptions &) {
    // don't need to worry about mutating the surface; InputImageSurface returns a fresh one
    BOOST_LOG_TRIVIAL(info) << "Pipeline Plot Output";
}
...
static void PipelineComparatorAttitude(std::ostream &os,
    ...
    float fractionCorrect = (numCorrect) / expected.size();
    float fractionIncorrect = (numIncorrect) / expected.size();
...
//                                          PipelineInputList &expected,
//                                          std::vector<PipelineOutput> &actual) {
//     int index1 = Prompt<int>("Index of first star");
//     int index2 = Prompt<int>("Index of second star");

//     Camera &camera = *expected[0]->InputCamera();
//     Stars &stars = *actual[0].stars;
...
//     BOOST_LOG_TRIVIAL(info) << "pair_distance_12 " << Angle(spatial1, spatial2);
//     BOOST_LOG_TRIVIAL(info) << "pair_distance_13 " << Angle(spatial1, spatial3);
//     BOOST_LOG_TRIVIAL(info) << "pair_distance_14 " << Angle(spatial1, spatial4);
//     BOOST_LOG_TRIVIAL(info) << "pair_distance_23 " << Angle(spatial2, spatial3);
//     BOOST_LOG_TRIVIAL(info) << "pair_distance_24 " << Angle(spatial2, spatial4);
//     BOOST_LOG_TRIVIAL(info) << "pair_distance_34 " << Angle(spatial3, spatial4);
// }
...
void PipelineComparison(const PipelineInputList &expected,
                        ...
                        const PipelineOptions &values) {
    if (actual.size() == 0) {
        BOOST_LOG_TRIVIAL(error) << "ERROR: No output! Did you specify any input images? Try --png or --generate.";
        exit(1);
    }
...
#define LOST_PIPELINE_COMPARE(precondition, errmsg, comparator, path, isBinary) do { \
        if (precondition) {                                             \
            UserSpecifiedOutputStream pos(path, isBinary, false);              \
            comparator(pos.Stream(), expected, actual, values);         \
        } else {                                                        \
            BOOST_LOG_TRIVIAL(error) << "ERROR: Comparator not applicable: " << errmsg; \
            exit(1);                                                    \
        }                                                               \
    } while (0)

    if (values.plotRawInput != "") {
        BOOST_LOG_TRIVIAL(info) << "Pipeline Plot Raw Input";
    ...
    if (values.plotInput != "") {
        BOOST_LOG_TRIVIAL(info) << "Pipeline Plot Input";
    ...
    if (values.plotExpected != "") {
        BOOST_LOG_TRIVIAL(info) << "Pipeline Plot Expected";
    ...
    if (values.printExpectedCentroids != "") {
        BOOST_LOG_TRIVIAL(info) << "Pipeline Expected Centeroids Output";
    ...
    if (values.printInputCentroids != "") {
        BOOST_LOG_TRIVIAL(info) << "Pipeline Input Centeroids Output";
    ...
    if (values.printActualCentroids != "") {
        BOOST_LOG_TRIVIAL(info) << "Pipeline Actual Centeroids Output";
    ...
    if (values.plotCentroidIndices != "") {
        BOOST_LOG_TRIVIAL(info) << "Pipeline Centeroids Indices";
    ...
    if (values.compareCentroids != "") {
        BOOST_LOG_TRIVIAL(info) << "Pipeline Compare Centeroids";
    ...
    if (values.compareStarIds != "") {
        BOOST_LOG_TRIVIAL(info) << "Pipeline Compare Star Ids";
    ...
    if (values.printAttitude != "") {
        BOOST_LOG_TRIVIAL(info) << "Pipeline Print Attitude";
    ...
    BOOST_LOG_TRIVIAL(info) << "Pipeline Print Attitude Completed";
    if (values.printExpectedAttitude != "") {
        BOOST_LOG_TRIVIAL(info) << "Pipeline Expected Print Attitude";
    ...
    BOOST_LOG_TRIVIAL(info) << "Pipeline Print Expected Attitude Completed";
    if (values.compareAttitudes != "") {
        BOOST_LOG_TRIVIAL(info) << "Pipeline Compare Attitude";
    ...
    BOOST_LOG_TRIVIAL(info) << "Pipeline Compare Attitude Completed";
    if (values.printSpeed != "") {
        BOOST_LOG_TRIVIAL(info) << "Pipeline Speed Attitude";
    ...
    BOOST_LOG_TRIVIAL(info) << "Pipeline Print Speed Completed";
...
// typedef void (*CatalogInspector)(Catalog &);

// static std::vector<CatalogStar *> PromptCatalogStars(Catalog &catalog, int howMany) {
//     std::vector<CatalogStar *> result;
//     for (int i = 0; i < howMany; i++) {
//         int name = Prompt<int>("Catalog name of " + std::to_string(i) + "-th star");
//         CatalogStar *star = findNamedStar(catalog, name);
//         if (star == NULL) {
...
// void InspectPairDistance(Catalog &catalog) {
//     auto stars = PromptCatalogStars(catalog, 2);

//     // TODO: not cout, prompt for an ostream in inspect and pass argument
//     BOOST_LOG_TRIVIAL(info) Angle(stars[0]->spatial, stars[1]->spatial);
// }

// void InspectPyramidDistances(Catalog &catalog) {
//     auto stars = PromptCatalogStars(catalog, 4);

//     BOOST_LOG_TRIVIAL(info) << "pair_distance_01 " << Angle(stars[0]->spatial, stars[1]->spatial);
//     BOOST_LOG_TRIVIAL(info) << "pair_distance_02 " << Angle(stars[0]->spatial, stars[2]->spatial);
//     BOOST_LOG_TRIVIAL(info) << "pair_distance_03 " << Angle(stars[0]->spatial, stars[3]->spatial);
//     BOOST_LOG_TRIVIAL(info) << "pair_distance_12 " << Angle(stars[1]->spatial, stars[2]->spatial);
//     BOOST_LOG_TRIVIAL(info) << "pair_distance_13 " << Angle(stars[1]->spatial, stars[3]->spatial);
//     BOOST_LOG_TRIVIAL(info) << "pair_distance_23 " << Angle(stars[2]->spatial, stars[3]->spatial);
// }

// void InspectTripleAngle(Catalog &catalog) {
//     auto stars = PromptCatalogStars(catalog, 3);

//     // TODO
// }

// void InspectFindStar(Catalog &catalog) {
...
//     int i = 0;
//     for (CatalogStar &curStar : catalog) {
//         if ((curStar.spatial - userSpatial).Magnitude() < tolerance) {
//             BOOST_LOG_TRIVIAL(info) << "found_star_" << i << " "  << curStar.name << std::endl;
//             BOOST_LOG_TRIVIAL(info) << "fonud_star_magnitude_" << i << " " << curStar.magnitude << std::endl;
//             i++;
//         }
//     }
//     if (i == 0) {
...
// void InspectPrintStar(Catalog &catalog) {
//     auto stars = PromptCatalogStars(catalog, 1);
//     float ra, de;
//     SpatialToSpherical(stars[0]->spatial, &ra, &de);

//     BOOST_LOG_TRIVIAL(info) << "star_ra " << RadToDeg(ra) << std::endl;
//     BOOST_LOG_TRIVIAL(info) << "star_de " << RadToDeg(de) << std::endl;
// }
